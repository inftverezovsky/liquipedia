...
export function normalizeDota2Tournament(input: {
  ...
  const participants = extractParticipants(input.wikitext);
  ...
  const normalizedMatches = allCandidates
    .map((c, idx) => normalizeMatchCandidate(c, input.title, String(idx)))
...
function extractMatchesFromParsedHtml(html: string, pageUrl: string): NormalizedMatch[] {
  ...
  function findSectionForElement(el: any): string {
    const $el = $(el);
    // Walk backwards through siblings and parents to find heading
  ...
  // 1. Extract from bracket matches (playoffs)
  $(".brkts-match").each((_, matchEl) => {
    const $match = $(matchEl);
    const opponents = $match.find(".brkts-opponent-entry");
    if (opponents.length < 2) return;

    const teamAName = opponents.eq(0).attr("aria-label")?.trim() || 
                      opponents.eq(0).find(".name").first().text().trim() || null;
    const teamBName = opponents.eq(1).attr("aria-label")?.trim() ||
                      opponents.eq(1).find(".name").first().text().trim() || null;
    ...
    const isWinA = opponents.eq(0).find(".brkts-opponent-win").length > 0 ||
                   opponents.eq(0).find(".brkts-opponent-entry-left").hasClass("brkts-opponent-win");
    const isWinB = opponents.eq(1).find(".brkts-opponent-win").length > 0 ||
                   opponents.eq(1).find(".brkts-opponent-entry-left").hasClass("brkts-opponent-win");

    // Extract date from popup
    ...
    // Determine stage from bracket section or heading
  ...
  // 2. Extract from matchlist items (group stage inline matches)
  $(".brkts-matchlist-match-flat, .brkts-matchlist-slot").each((_, matchEl) => {
    const $match = $(matchEl);
    const opponents = $match.find(".brkts-opponent-entry");
    if (opponents.length < 2) return;
    ...
    const $popup = $match.find(".brkts-popup").first();
    const timer = $popup.find(".timer-object").first();
    const timestamp = timer.attr("data-timestamp");
    const dateText = timer.text().trim() || null;
    ...
    const stage = findSectionForElement(matchEl) || null;
    ...
    matches.push({
      ...
      format: null,
      status: timer.attr("data-finished") === "finished" ? "finished" : null,
      court: null,
      sourceUrl: pageUrl,
      rawText: $.html(matchEl)?.slice(0, 2500)
    });
...
function extractParticipants(wikitext: string): NormalizedParticipant[] {
  const section =
